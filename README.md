...
## PR opnieuw aanmaken (GitHub)

Als een PR vastloopt/conflicts blijft houden:
1. Sluit de oude PR op GitHub (niet mergen).
2. Zorg dat je branch `work` op GitHub de nieuwste commit heeft.
3. Open een nieuwe PR van `work` -> `main`.
4. Controleer dat er maar 1 recente "clean snapshot" commit in de PR staat.

Tip: als je lokaal werkt, gebruik:
```bash
git push --force-with-lease origin work
```
alleen als je bewust de branchgeschiedenis hebt herschreven.
